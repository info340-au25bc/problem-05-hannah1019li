...
function compoundInterest(principle, annualRate, years) {
  return principle * Math.exp(annualRate * years);
}
...
function containsQueenOfHearts(cards) {
  for (const c of cards) {
    if (c.suit === 'hearts' && c.rank === 12) return true;
  }
  return false;
}
...
function getHighCard(cards) {
  ...
  for (let i = 1; i < cards.length; i++) {
    if (cards[i].rank > best.rank) best = cards[i];
  }
  return best;
}
...
function hasPair(cards) {
  ...
  for (const rk in counts) {
    if (counts[rk] >= 2) {
      const numRk = Number(rk);
      if (numRk > bestPairRank) {
        bestPairRank = numRk;
      }
    }
  }
